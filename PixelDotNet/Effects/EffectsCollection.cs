            ...
            new Quadruple<string, string, Version, string>[]
                {
                    ...
                    QuadrupleUtils.Create("FilmEffect", "FilmEffect", new Version(1, 0, int.MaxValue, int.MaxValue), UnstableMessage),

                    // Ed Harvey's Threshold effect, v1.0, uses a timer in its config dialog that NullRef's on us
                    QuadrupleUtils.Create("EdHarvey.Edfects.Effects", "ThresholdEffect", new Version (1, 0, int.MaxValue, int.MaxValue), UnstableMessage),

                    // BoltBait's InkSketch, which is now built-in to Paint.NET
                    QuadrupleUtils.Create("InkSketch", "EffectPlugin", new Version(1, 0, int.MaxValue, int.MaxValue), BuiltInMessage),

                    // Portrait, which is now built-in to Paint.NET ("Soften Portrait")
                    QuadrupleUtils.Create("PortraitEffect", "EffectPlugin", new Version(1, 0, int.MaxValue, int.MaxValue), BuiltInMessage),

                    // Reduce Noice, which is now built-in to Paint.NET
                    QuadrupleUtils.Create("HistogramEffects", "ReduceNoiseEffect", new Version (1, 1, 0, 0), BuiltInMessage),

                    // Fragment, which is now built-in
                    QuadrupleUtils.Create("EdHarvey.Edfects.Effects", "FragmentEffect", new Version(3, 20, int.MaxValue, int.MaxValue), BuiltInMessage),

                    // Posterize, which is now built-in
                    QuadrupleUtils.Create("EdHarvey.Edfects.Effects", "PosterizeEffect", new Version(3, 31, int.MaxValue, int.MaxValue), BuiltInMessage),
                    ...
                    QuadrupleUtils.Create("pyrochild.effects.splatter", "Splatter", new Version(1, 0, int.MaxValue, int.MaxValue), UnstableMessage),
                    QuadrupleUtils.Create("zachwalker.CurvesPlus", "CurvesPlus", new Version(2, 4, int.MaxValue, int.MaxValue), UnstableMessage),

                    // Older versions of these were accessing code in paintdotnet.exe, or were using the EventHandler<T> that was moved
                    QuadrupleUtils.Create("pyrochild.effects.gradientmapping", "GradientMapping", new Version(2, 1, int.MaxValue, int.MaxValue), UnstableMessage),

                    // GREYCstoration, has known stability issues, and is also superceded by the built-in "Reduce Noise" anyway
                    QuadrupleUtils.Create("GREYCstoration", "BaseEffect", new Version(0, 9, int.MaxValue, int.MaxValue), UnstableMessage),
                    QuadrupleUtils.Create("GREYCstoration", "RestoreEffect", new Version(0, 9, int.MaxValue, int.MaxValue), UnstableMessage)
                };
        ...
        private static List<Type> GetEffectsFromAssemblies(Assembly[] assemblies, IList<Triple<Assembly, Type, Exception>> errorsResult)
        {
            ...
            foreach (Type effectType in effects)
            {
                ...
                if (bannedEx != null)
                {
                    ...
                    errorsResult.Add(TripleUtils.Create(effectType.Assembly, effectType, bannedEx));
                }
            }
        ...
        private static Type[] GetTypesFromAssembly(Assembly assembly, IList<Triple<Assembly, Type, Exception>> errorsResult)
        {
            ...
            catch (ReflectionTypeLoadException rex)
            {
                ...
                foreach (Exception loadEx in rex.LoaderExceptions)
                {
                    ...
                    errorsResult.Add(TripleUtils.Create<Assembly, Type, Exception>(assembly, null, loadEx));
                }
